---
name: brainstorming
description: "在尚未进入任何 /opsx:* 命令流程时，于创造性工作之前使用——通过纯文字对话帮助用户明确需求、发散思路、比较方向并形成清晰共识。若对话已处于 /opsx:pre-design、/opsx:design、/opsx:explore、/opsx:ff 等命令中，由命令自身的澄清环节负责，不要触发本技能。"
---
...
每个项目都应该先经过这一步。一个小功能、一次行为修改、一个配置调整，甚至只是一个想法验证，都值得先明确问题和方向。真正简单的项目可以很快完成探索，但不能跳过。（例外：一句话能说清 diff 的微小修改，按项目 CLAUDE.md 的路由规则直接处理，不属于本技能的触发范围。）

## 与 OPSX 工作流的边界

- 本技能服务于**未立项**的自由探讨。一旦对话已处于任何 `/opsx:*` 命令流程中（pre-design / design / explore / ff 等），**不要触发或叠加本技能**——这些命令自带澄清环节，重复触发会造成双重提问甚至互相卡门。
- 探讨中形成"要正式立项"的共识时，引导用户走 `/opsx:pre-design`（决定要做、需要决策卡）或 `/opsx:explore`（还要继续想）。本技能自身仍然不产出任何文档。
